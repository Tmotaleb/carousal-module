...
//generate 100 listings
  for (var i = 0; i < 3; i++) {
    ...
    // var moreSpaceImg = randomImage(moreSpaces, 1);
    ...
    function checkMin(member) {
      if (member <= 1) {
        return `Private office complete with office furniture, overhead storage and file cabinets. Enjoy 24/7 access to all amenities. Conference rooms are available through booking and whiteboard walls are provided upon request.`
      } else {
        return `Spacious workspace complete with office furniture, overhead storage and file cabinets. Enjoy 24/7 access to all amenities. Conference rooms are available through booking and whiteboard walls are provided upon request.`
      }
    }
      ...
      moreSpacesInfo:[
        // {image: moreSpaceImg, location: location, office: office, member: member, info: checkMin(member)}
   ...
   if (moreSpaces.length >= 1) {
     for (var i = 0; i < moreSpaces.length; i++) {
       fakeData.moreSpacesInfo.push({image: moreSpaces[i], location: faker.address.streetAddress(), office: faker.random.number({min: 100, max: 500}), member: faker.random.number({min: 1, max: 10}), info: checkMin(member)})
     }
   } else {
